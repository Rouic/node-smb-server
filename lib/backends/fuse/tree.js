...
FSTree.prototype.rename = function (oldName, newName, cb) {
  ...
  var compOld = Path.join(this.share.path, oldName);
  var compNew = Path.join(this.share.path, newName);
  
  	//FUSE DIRECTORY COPY IMPLEMENTATION
  	
  	//Fuse cannot rename directories whilst they are IN Fuse, so this essentitally copies the entire thing OFF fuse, 
  	//renames it in the process and then copies it back discarding the old ones!
  	
	if(fs.lstatSync(compOld).isDirectory()){  //if item is dir
		ncp(compOld, __dirname+'/../../../smbtemp/'+newName, function(err){ //copy dir to temp directory with new name
			if (err){
				cb(SMBError.fromSystemError({message: 'cannot delete root directory'}));
			   return console.error(err);
			} else {	 	 	 	
				if (compOld && compOld != '/') {
				    rimraf(compOld, function(error){ //delete old dir
					    if(error){
						    SMBError.systemToSMBErrorTranslator(cb)
					    } else {
							ncp(__dirname+'/../../../smbtemp/'+newName, compNew, function(err){ //copy new dir back to FUSE
								if (err) {
							    	cb(SMBError.fromSystemError({message: 'cannot delete root directory'}));
									return console.error(err);
							 	} else {
									rimraf(__dirname+'/../../../smbtemp/'+newName, function(error){ //delete temp dir
										cb();
									});
								}
							});	 				    
